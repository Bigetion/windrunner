...
// ─── space-x-* / space-y-* ───────────────────────────────────────────────────
// Uses the lobotomized owl selector: > :not(:first-child) / > :not(:last-child)
...
export function buildDivideDeclaration(baseToken, theme) {
  ...
  if (matchX) {
    ...
    if (val !== undefined) return `--tw-divide-x-reverse: 0; border-inline-end-width: calc(${val} * var(--tw-divide-x-reverse)); border-inline-start-width: calc(${val} * calc(1 - var(--tw-divide-x-reverse)));`;
  }
  ...
  if (matchY) {
    ...
    if (val !== undefined) return `--tw-divide-y-reverse: 0; border-bottom-width: calc(${val} * var(--tw-divide-y-reverse)); border-top-width: calc(${val} * calc(1 - var(--tw-divide-y-reverse)));`;
  }

  // divide-style
  if (BORDER_STYLE_MAP[key]) return `border-style: ${BORDER_STYLE_MAP[key].replace("border-style: ", "").replace(";", "")};`;

  // divide-none / divide-solid etc. (border-style shorthands)
  ...
  if (key.startsWith("opacity-")) {
    ...
    if (val !== undefined) return `--tw-divide-opacity: ${val};`;
  }
